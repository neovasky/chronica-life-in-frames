...
class ChornicaTimelinePlugin extends obsidian.Plugin {
  ...
  async handleFileDelete(file) {
    ...
    if (eventsRemovedCount > 0) {
      ...
      new obsidian.Notice(
        `Removed ${eventsRemovedCount} event link(s) from timeline.`
      );
    }
  ...
  async cleanInvalidEvents() {
    ...
    if (invalidRemovedCount > 0) {
      ...
      this.refreshAllViews();
      new obsidian.Notice(
        `Cleaned ${invalidRemovedCount} invalid event link(s).`
      );
    }
  }
}
...
class ChornicaEventModal extends obsidian.Modal {
  ...
  constructor(
    app,
    plugin,
    preselectedWeekKey = null // Expecting YYYY-WXX format now
  ) {
    super(app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    const singleDateSetting = new obsidian.Setting(singleDateContainer).setName(
      "Date"
    );
    ...
    const startDateSetting = new obsidian.Setting(rangeDateContainer).setName(
      "Start Date"
    );
    ...
    const endDateSetting = new obsidian.Setting(rangeDateContainer).setName(
      "End Date"
    );
  ...
  async saveEvent() {
    ...
    if (this.isDateRange) {
      ...
      if (endDate < startDate) {
        new obsidian.Notice(
          "End date cannot be before start date. Event not saved."
    ...
    try {
      ...
      new obsidian.Notice(
        "Event saved to settings, but failed to create/update note."
      );
    }
        ...
        btn
          ...
          .onClick(async () => {
            ...
            if (
              ...
              new obsidian.Notice(
                `Event type "${this.newTypeName}" already exists.`
              );
              return;
            }
...
class ManageEventTypesModal extends obsidian.Modal {
  ...
  onOpen() {
    ...
    new obsidian.Setting(addSection).addButton((button) =>
      button
        ...
        btn
          ...
          .onClick(async () => {
            ...
            if (typeIndex !== -1) {
              ...
              new obsidian.Notice(
                `Error: Could not find type with ID ${type.id}.`
              );
            }
          })
      )
...
class ChornicaTimelineView extends obsidian.ItemView {
  ...
  applyEventStyling(cell, weekKey) {
    ...
    // Tooltip Management:
    cell.dataset.cellWeekNum || "";
    cell.dataset.cellIsoYear || "";
    cell.dataset.cellDateRange || "";
...
class ChornicaSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const createInfoBubbleWithCustomTooltip = (
      setting, // The setting to attach the bubble to
      placeholderText
    ) => {
        ...
        button
          ...
          .onClick(async () => {
            if (
              ...
              this.refreshAllViews(); // Update views
              new obsidian.Notice(
                "Event types reset to default. All events cleared."
              );
            }
          });
      });
