    ...
    async saveEvent() {
        ...
        if (this.isEditMode && this.originalEvent) {
            ...
            if (eventIndex !== -1) {
                ...
                const noteProcessed = await this.createOrUpdateEventNote(eventToUpdate, finalEventName, startDate, endDate);
                if (noteProcessed && eventToUpdate.notePath) ;
                else if (!noteProcessed) {
                    // Failed to process note, but event data in settings is updated
                    new obsidian.Notice("Event updated in settings, but failed to update/create note file.");
                }
                await this.plugin.saveSettings();
                new obsidian.Notice(`Event "${nameForEventObject}" updated.`);
            }
            else {
                new obsidian.Notice("Error: Original event not found for update. Could not save changes.");
                this.close();
                this.refreshViews();
                return;
            }
