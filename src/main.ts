...
class ChronosTimelineView extends ItemView {
  ...
  fitToScreen(): void {
    ...
    // Calculate the minimum desirable size
    const minDesirableWidth = Math.min(availW, 1000); // Cap at 1000px
    const minDesirableHeight = Math.min(availH, 600); // Cap at 600px
    ...
    if (verticalMarkers) verticalMarkers.style.transform = "";
