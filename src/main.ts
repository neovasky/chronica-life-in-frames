...
class ChornicaEventModal extends Modal {
  ...
  constructor(
    ...
    if (typeof targetDataOrEvent === "string") {
      ...
      this.originalEvent = targetDataOrEvent as ChronicaEvent; // Explicitly cast
...
class ChornicaTimelineView extends ItemView {
  ...
  renderWeeksGrid(container: HTMLElement): void {
    ...
    if (settings.showBirthdayMarker) {
      ...
      const birthWeekGridIndex = 0;

      const birthdayMarkerContainer = container.createEl("div", {
        cls: "chronica-birthday-marker-container",
      });

      // Calculate position based on gridIndex 0
      // This effectively positions it at the start of the week axis
      ...
      cakeEl.setAttribute(
        "title",
        `${birthMonthName} ${bDay}, ${bYear} (Your Birthday)`
      );
    }
...
class ChornicaSettingTab extends PluginSettingTab {
  ...
  - \${endDate_YYYY}, \${endDate_MM}, \${endDate_DD}, \${endDate_MMMM}, \${endDate_MMM}, \${endDate_YY} (for actual end date)`;
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Show links to associated weekly or event notes in the tooltip (Expanded view only). May slightly impact performance on hover if snippets are fetched (future)."
      )
      .addToggle((toggle) =>
        toggle
