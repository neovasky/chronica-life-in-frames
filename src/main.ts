...
export default class ChronosTimelinePlugin extends Plugin {
  ...
  async scanVaultForEvents(): Promise<void> {
    ...
    if (
      (!this.settings.notesFolder || this.settings.notesFolder.trim() === "") &&
      (!this.settings.useSeparateFolders ||
        !this.settings.eventNotesFolder ||
        this.settings.eventNotesFolder.trim() === "")
    ) {
      ...
      if (
        ...
        const eventFiles = files.filter((file) =>
          file.path.startsWith(normalizedEventFolder)
        );
...
class ChronosEventModal extends Modal {
  ...
  async saveEvent(): Promise<void> {
    ...
    if (this.isDateRange && this.selectedEndDate) {
      ...
      const fileName = this.plugin.formatFileName(
...
class ChronosTimelineView extends ItemView {
   ...
   */
  async updateZoomLevel(): Promise<void> {
   ...
   */
  async prefetchAllEventData(): Promise<
    Map<string, { type: string; description: string; color: string }>
  > {
      ...
      try {
        ...
        if (metadata.startDate) {
          ...
          const endDate = metadata.endDate
            ...
            : startDate;
          const weekKeys = this.plugin.getWeekKeysBetweenDates(
            startDate,
            endDate
          );
          ...
          const rangeMatch = file.basename.match(
            /(\d{4}-W\d{2})_to_(\d{4}-W\d{2})/
          );
          if (rangeMatch) {
            const startWeekKey = rangeMatch[1];
            const endWeekKey = rangeMatch[2];
            const startDate = this.plugin.getStartOfISOWeek(
              new Date(
                parseInt(startWeekKey.split("-W")[0]),
                0,
                (parseInt(startWeekKey.split("-W")[1]) - 1) * 7 + 4
              )
            ); // Approx start date
            const endDate = this.plugin.getStartOfISOWeek(
              new Date(
                parseInt(endWeekKey.split("-W")[0]),
                0,
                (parseInt(endWeekKey.split("-W")[1]) - 1) * 7 + 4
              )
            ); // Approx end date
            const weekKeys = this.plugin.getWeekKeysBetweenDates(
              startDate,
        ...
        console.warn(
          "Chronica: Error processing potential event file:",
          file.path,
          error
        );
      }
      ...
      events.forEach((eventData) => {
        ...
        if (parts.length === 2 && parts[0].includes("W")) {
          ...
          try {
            ...
            console.warn(
              "Chronica: Error parsing fallback range event:",
              eventData,
              e
            );
          }
    ...
    processFallbackEvents(
      ...
      "Major Life",
      this.plugin.settings.majorLifeColor
    );
    processFallbackEvents(
      this.plugin.settings.blueEvents,
      "Travel",
      this.plugin.settings.travelColor
    );
    processFallbackEvents(
      this.plugin.settings.pinkEvents,
      "Relationship",
      this.plugin.settings.RelationshipColor
    );
    processFallbackEvents(
      this.plugin.settings.purpleEvents,
      "Education/Career",
      this.plugin.settings.CareerColor
    );
    ...
    if (
      ...
      for (const customType of this.plugin.settings.customEventTypes) {
        if (this.plugin.settings.customEvents[customType.name]) {
          processFallbackEvents(
            this.plugin.settings.customEvents[customType.name],
            customType.name,
            customType.color
          );
        }
   ...
   */
  async renderWeeksGrid(container: HTMLElement): Promise<void> {
    container.empty();
      ...
      for (let cellIndex = 0; cellIndex < 52; cellIndex++) {
        ...
        if (eventInfo) {
            ...
            // Add a generic class if using custom color, or specific if matching built-in
             if (["Major Life", "Travel", "Relationship", "Education/Career"].includes(eventInfo.type)) {
                 // Already handled by specific class like chronica-event-major-life
             } else {
            ...
            const prevTitle = cell.getAttribute("title") || "";
             const eventTitle = `${eventInfo.type}: ${eventInfo.description}`; // Simplified tooltip for now
        ...
        if (isCurrentWeek) {
            cell.addClass("present");
             if (!hasEvent) cell.addClass("chronica-cell-present");
        } else if (cellStartDate < now) {
             cell.addClass("past");
             if (!hasEvent) cell.addClass("chronica-cell-past");
         } else {
             cell.addClass("future");
             if (!hasEvent) cell.addClass("chronica-cell-future");
         }
  ...
  renderChartsTab(container: HTMLElement): void {
    ...
    if (totalSeasonEvents > 0) {
      ...
      // Draw axis lines
      const axisPoints = [
        { x: CENTER, y: CENTER - OUTER_RADIUS }, // Winter
        { x: CENTER + OUTER_RADIUS, y: CENTER }, // Spring
        { x: CENTER, y: CENTER + OUTER_RADIUS }, // Summer
      ...
      const labelPoints = [
        { x: CENTER, y: CENTER - OUTER_RADIUS - 20 }, // Winter (above)
        { x: CENTER + OUTER_RADIUS + 30, y: CENTER }, // Spring (right)
        { x: CENTER, y: CENTER + OUTER_RADIUS + 20 }, // Summer (below)
